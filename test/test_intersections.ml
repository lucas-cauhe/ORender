...
let fig_properties =
  { emission = Rgb.rgb_of_values 255. 0. 0.
  ; coefficients =
      Rgb.rgb_of_values 1. 0. 0., Rgb.rgb_of_values 1. 0. 0., Rgb.rgb_of_values 1. 0. 0.
  ; refraction = 1.
  }
;;

let one_intersection =
  ref
    (Intersects
       [ { distance = 1.
         ; surface_normal = Direction.from_coords 1. 1. 1.
         ; intersection_point = Point.from_coords 1. 1. 1.
         }
       ])
;;

let two_intersection =
  ref
    (Intersects
       [ { distance = 1.
         ; surface_normal = Direction.from_coords 1. 1. 1.
...
let testable_intersection =
  let module M = struct
    ...
    let pp fmt = function
      | Zero -> Format.fprintf fmt "No intersection\n"
      | Intersects il ->
        List.iter
          (fun f ->
            Format.fprintf
              ...
              "{ dist = %f; snormal = %s } | "
              f.distance
              (Direction.string_of_direction f.surface_normal))
          il;
        print_newline ()
    ;;
...
let test_sphere_no_intersection _ =
  let sphere = sphere (Point.from_coords 4. 0. 0.) 1. fig_properties in
  let ray =
...
let test_sphere_one_intersection _ =
  ...
  let r = ref 10. in
  let sphere = sphere center !r fig_properties in
  ...
  let k = ref 5. in
  (* cte para tomar el origen *)
  let w =
    Direction.from_coords
      (!r *. sin !lat *. cos !acimut)
      (!r *. sin !lat *. sin !acimut)
      (!r *. cos !lat)
  in
  ...
  check
    testable_intersection
    "Intersections differ from 1"
    !one_intersection
    (intersects sphere ray)
;;

let test_sphere_two_intersection _ =
  let sphere = sphere (Point.from_coords 4. 3. 5.) 4. fig_properties in
  let ray =
    { ray_origin = Point.from_coords 0. 0. 0.
    ; ray_direction = Direction.from_coords 1. 1. 1.
    }
...
let test_sphere_one_intersection_from_origin _ =
  let sphere = sphere (Point.from_coords 4. 3. 5.) 10. fig_properties in
  let ray =
    { ray_origin = Point.from_coords 0. 0. 0.
    ; ray_direction = Direction.from_coords 1. 1. 1.
    }
...
let test_plane_parallel _ =
  ...
  let perp_plane = plane plane_normal (Point.from_coords 1. 1. 1.) fig_properties in
  check
    testable_intersection
    "Ray & Plane intersect (not orthogonal)"
    Zero
    (intersects perp_plane ray)
;;
...
let test_plane_intersects _ =
  ...
  in
  let plane =
    plane (Direction.from_coords (-1.) 0. 0.) (Point.from_coords 3. 2. 1.) fig_properties
  in
  check
    testable_intersection
    "Ray & Plane don't intersect"
    !one_intersection
...
let test_triangle_intersects _ =
  ...
  let triangle =
    triangle
      (Point.from_coords 2. 0. 2.)
      (Point.from_coords 2. 2. 0.)
      (Point.from_coords 0. 2. 2.)
      fig_properties
  in
  match triangle with
...
let test_triangle_not_intersects _ =
  ...
  let triangle =
    triangle
      (Point.from_coords 2. 0. 1.)
      (Point.from_coords 2. 2. 1.)
      (Point.from_coords 4. 0. 1.)
      fig_properties
  in
  match triangle with
  | None -> failwith "bad triangle definition"
...
let test_sphere =
  ...
  ; ( "test_sphere_one_intersection_from_origin"
    , `Quick
    , test_sphere_one_intersection_from_origin )
