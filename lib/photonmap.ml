...
let weight_scene_lights ls num_photons =
  ...
  List.fold_left
    (fun acc light_source ->
      ...
      in
      acc
      @ List.init (s |> int_of_float) (fun _ -> Light.sample_light_source light_source, s))
    []
    ls
;;
  ...
  | fig, Figures.Intersects (ir :: _) ->
    (match Brdf.russian_roulette (Figures.get_figure fig) with
     ...
     | roulette_result, roulette_prob ->
       ...
       let next_photon =
         Photon.photon photon_radiance ir.intersection_point photon_ray.ray_direction
       in
  ...
  let rec rec_random_walk photons = function
    ...
    | (next_light, emitted_photons) :: rest_lights ->
      ...
      let next_photon_flux =
        Rgb.value_prod (Light.power next_light) (4. *. Float.pi /. emitted_photons)
      in
      let initial_photon =
        Photon.photon
          next_photon_flux
