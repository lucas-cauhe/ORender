...
let photon_search
  ...
  List.filter (Photon.in_surface surface) knn |> BatList.take 1000, sqrt radius
;;
    ...
    let global_light_contribution =
      density_estimation
        (Photon.brdf
           ...
           wi.ray_direction
           (Diffuse, prob))
        (Gaussian { intersection_position = ir.intersection_point; smooth = knn_radius })
...
let rec nee_photonmap scene ls photonmap texture_map wi =
  ...
  if is_delta then (
    ...
    let knn, _knn_radius =
      photon_search photonmap ir.intersection_point 0.01 (Figures.get_figure fig)
    in
    let diffuse_brdf =
      Brdf.brdf
        (Figures.get_figure fig)
