...
module Ppm = struct
  ...
  let read_header ic =
    let rec header_reader rem config =
      try
        ...
        | 2, _ ->
          header_reader
            (rem - 1)
            (Scanf.sscanf line "%d %d" (fun w h -> { config with width = w; height = h }))
    ...
    header_reader
      3
      { ppm_version = "654"; max = 1.; ppm_max = 1; width = 10; height = 10 }
  ;;
  ...
  let config_of_values v max ppm_max w h =
    { ppm_version = v; max; ppm_max; width = w; height = h }
  ;;
