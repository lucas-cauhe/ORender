...
let identity dim =
  Array.init_matrix dim dim (fun i j ->
    if i = j then
      1.
    else
      0.)
...
let multiply m1 m2 =
  let x0 = Array.length m1
  and y0 = Array.length m2
  and x1 = Array.length m1.(0)
  and y1 = Array.length m2.(0) in
  if x1 <> y0 then
    None
  else (
...
let inverse mat =
  ...
  if det = 0.0 then (
    ...
    let n = Array.length mat in
    Some (Array.init n (fun i -> Array.init n (fun j -> adjugate.(i).(j) /. det)))
  )
;;
