...
functor
  ...
  struct
    ...
    module Range = struct
      ...
      let create lb rb = { lb; rb }

      (* get bounds in given dimension *)
      let get_bounds ndim r = P.get_coord ndim r.lb, P.get_coord ndim r.rb
      ...
      let min_corner () = P.create (A.init P.ndims (fun _ -> -.max_float))
      let max_corner () = P.create (A.init P.ndims (fun _ -> max_float))
      let max_box () = { lb = min_corner (); rb = max_corner () }
      ...
      let all_corners box =
        let n = P.ndims in
        let corners = ref [] in
        let rec generate idx acc =
          if idx = n then
            corners := P.create acc :: !corners
          else (
            generate (idx + 1) (Array.append acc [| P.get_coord idx box.lb |]);
            generate (idx + 1) (Array.append acc [| P.get_coord idx box.rb |])
          )
        in
        generate 0 [||];
        !corners
      ...
      let is_inside small big =
        List.for_all (fun corner -> point_is_inside corner big) (all_corners small)
      ;;
      ...
      let intersect small big =
        let small_corners = all_corners small in
        let big_corners = all_corners big in
        try
          (* Check if any corner of small is inside big *)
          List.iter
            (fun corner -> if point_is_inside corner big then raise Intersect)
            small_corners;
          (* Check if any corner of big is inside small *)
          List.iter
            (fun corner -> if point_is_inside corner small then raise Intersect)
            big_corners;
          false
        with
        | Intersect -> true
    ...
    let median get_coord a =
      ...
      if n mod 2 = 1 then
        ...
        0.5 *. (get_coord a.(n / 2) +. get_coord a.((n / 2) - 1))
    ;;
    ...
    let left_count p a =
      ...
      try
        for i = 0 to n - 1 do
          if not (p (A.unsafe_get a i)) then raise (Finished i)
        done;
        n (* all OK *)
      with
      | Finished i -> i (* some KO *)
    ;;
    ...
    let search range t =
      let rec loop depth acc curr_box = function
        | Empty -> acc
        | Leaf p ->
          if Range.point_is_inside p range then
            p :: acc
          else
            acc
        | Node (left, med, right) ->
