...
let shadow_ray scene intersection ls = 
  let ls_center = match ls.ls_type with
  | Area(_) -> Point.from_coords 0. 0. 0. (* This function should never be called with area light *) 
  | Point(p) -> p in
  match  Direction.between_points ls_center intersection.intersection_point |> Direction.normalize with
  | None -> ls.ls_power
