...
let scene_limits (scene : Figures.scene) : Point.point_t * Point.point_t = 
  let min_max_point (min_point, max_point) point =
    (
      ...
      Point.from_coords (max (Point.x max_point) (Point.x point) ) (max (Point.y max_point) (Point.y point) ) (max (Point.z max_point) (Point.z point) )
    ) in

  let min_max_figure min max fig = 
    List.fold_left min_max_point (min, max) (Figures.vertices fig) in
...
let largest_axis (lower_bound : Point.point_t) (upper_bound : Point.point_t) (scene : Figures.scene) : axis * float = 
  ...
  let barycenters = List.fold_left (fun acc fig -> Point.sum acc (Figures.barycenter (Figures.get_figure fig))) (Point.from_coords 0. 0. 0.) scene in
...
let split_scene_by_axis (scene : Figures.scene) (a : axis) (mid_point : float) : Figures.scene * Figures.scene =
...
let rec split_largest_axis (scene : Figures.scene) : Figures.scene =
  ...
  let scene_bbox = Figures.cuboid scene_min scene_max (Rgb.zero ()) ~coefficients:(Rgb.zero (),Rgb.zero (), Rgb.zero ()) in
...
let split_scene scene algo =  
  ...
  let new_scene_wout_planes = match algo with
  | LargestAxis -> split_largest_axis scene_wout_planes
  | Sah -> split_sah scene_wout_planes in
