...
let parse_vertex line =
  Scanf.sscanf line "v %f %f %f" (fun x y z -> { x; y; z })

let parse_normal line =
  Scanf.sscanf line "vn %f %f %f" (fun nx ny nz -> { nx; ny; nz })

let parse_face line =
  Scanf.sscanf line "f %d/%d/%d %d/%d/%d %d/%d/%d" (fun v1 _ _ v2 _ _ v3 _ _ -> { v1; v2; v3 })
...
let convert_to_scene (vertexs, faces) =
  ...
  let convert_face f =
    ...
    Figures.Figure(Figures.triangle (convert_vertex v1) (convert_vertex v2) (convert_vertex v3) (Colorspace.Rgb.rgb_of_values 0.75 0.75 0.) ~coefficients:(Colorspace.Rgb.rgb_of_values 0.75 0.75 0.) |> Option.get)
  in
