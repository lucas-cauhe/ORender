...
let _photonmap_pixel_color cam (row, col) ls scene photons pool =
...
let pathtracing_pixel_color cam (row, col) scene light_sources pool =
  let pip_arr = BatArray.of_list (points_in_pixel cam (row, col) !num_points) in
  let compute_pixel_color ind =
    Direction.between_points pip_arr.(ind) (cam_origin cam)
    |> Direction.normalize
...
let () =
  ...
  (* let _photons = random_walk my_scene light_sources 100000 pool in *)
